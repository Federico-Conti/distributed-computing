...
class Node:
    ...
    b: int  # Maximum number of blocks per peer
    ...
    def __post_init__(self):
        ...
        self.blocks_per_peer: dict[Node, int] = {}  # Map of blocks per peer (peer -> number of stored blocks)
    ...
    def schedule_next_download(self, sim: Backup):
        ...
        for block_id, (held_locally, peer) in enumerate(zip(self.local_blocks, self.backed_up_blocks)):
         if not held_locally and peer is not None and peer.online and peer.current_upload is None:
            # Schedule the restore from peer to self for block_id
          ...
          for _ in range(self.b):
            if (peer is not self and peer.online and peer.current_upload is None and peer not in self.backed_up_blocks
                ...
                if block_id is not None:
                    # Schedule the backup of block_id from peer to self
...
class BlockRestoreComplete(TransferComplete):
    ...
    def update_block_state(self):
        ...
        if sum(owner.local_blocks) == owner.k:  # we have exactly k local blocks, we have all of them then
            owner.local_blocks = [True] * owner.n  # Mark all blocks as present
            owner.failed = False  # Clear the failure state
